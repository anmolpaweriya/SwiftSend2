...
function App() {
    ...
    function _(id: string) { return document.getElementById(id) }
    ...
    async function getProfilePic(id: string) {
        const data = await fetch(`https://api.multiavatar.com/${id}?apikey=HUifqakYcTK94I`)
    ...
    function openUser(id: string, type: 'send' | 'receive', fileName: string = "", fileSize: number = 0) {
        setOpenedUser(pre => ({ ...pre, [id]: { id, type, fileName, fileSize, offset: 0 } }))
    }
    ...
    function toggleOpenUser(id: string) {
        if (Object.keys(openedUser).includes(id))
            closeUser(id)
    ...
    function createConnection(id: string) {
        ...
        dataChannels.current[id].onmessage = event => handleFileTransfer(event, id)
        peer.ondatachannel = event => { dataChannels.current[id] = event.channel }
    ...
    function handleFileTransfer(event: MessageEvent, id: string) {
        ...
        setOpenedUser(pre => {
            if (downloadedChunks.current[id].receivedDataSize < downloadedChunks.current[id].fileSize) {
                // todo
                // socket?.emit('request-accepted', id)
    ...
    return (
        <>
            ...
            <section className='h-[300px] text-black grid sm:grid-cols-[1fr_3fr] py-10 '>
                ...
                <svg viewBox="0 0 900 900" className="w-[600px] absolute top-[-300px] left-[-250px] z-[-1]">
...
M188.8 -191C261.6 -116 349.3 -58 346.8 -2.5C344.4 53 251.7 106.1 178.9 147.7C106.1 189.4 53 219.7 -14.8 234.6C-82.7 249.4 -165.5 248.8 -227.8 207.1C-290.1 165.5 -332.1 82.7 -327.7 4.4C-323.3 -74 -272.7 -148 -210.4 -223C-148 -298 -74 -374 -8 -366C58 -358 116 -266 188.8 -191" dur="4s" repeatCount="indefinite"></animate>
                ...
                <div className='top-[50px] left-[75px] justify-self-start mt-[30px] ml-[50px] '>
                    ...
                    <h1 className="font-semibold text-center text-xl drop-shadow-md">{username}</h1>
                </div>
                ...
                <div className="w-full flex flex-col gap-3 justify-center items-center max-sm:mt-[50px]">
                    ...
                    <div className="text-white font-semibold flex gap-2 items-center justify-center">
                        ...
                        <button className="text-xl rounded-lg p-2 bg-[#333] hover:bg-blue-400 transition-all hover:text-black"
            ...
            <section className="mt-20">
                ...
                <div className=" flex gap-5 flex-wrap m-5">
                    {Object.values(users).map(user => {
                        ...
                        >
                            <div
                                className={"transition-all duration-300 h-14 flex relative  rounded-full overflow-hidden "
                                    + (Object.keys(openedUser).includes(user.id) ? 'w-[300px]' : "w-14")
                                    + (openedUser[user.id]?.type == 'receive' ? " bg-[#0f0] " : " bg-[#5BB6FF] ")
                                }
                            >
                                ...
                                <div
                                    ...
                                    <ReactSVG src={`data:image/svg+xml;base64,${btoa(user.image)}`} />
                                </div>
                                            ...
                                            pendingRequests[user.id] == 'downloading'
                                                ...
                                                (
                                                    Object.keys(pendingRequests).includes(user.id) ?
                                                        <p
                                                            className="bg-white rounded-lg text-center w-full"
                                                        >
